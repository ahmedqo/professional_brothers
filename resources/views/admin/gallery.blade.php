...
@section('content')
    @include('admin.commun.header', ['title' => __('Gallery')])
    <section class="w-full container mx-auto p-4 flex flex-col gap-4">
        <div class="flex flex-col gap-4">
            <div class="w-full flex items-center">
                <div class="w-full lg:w-[300px]">
                    <select id="filter" x-select placeholder="{{ __('Filter by services') }}"
                        ...
                        <option value="glass facade cleaning">
                            {{ __('Glass Facade Cleaning') }}
                        </option>
                        <option value="stone facade cleaning in kuwait">
                            {{ __('Stone Facade Cleaning in Kuwait') }}
                        </option>
                        <option value="exterior floor cleaning">
        ...
        <form id="deleteForm" action="{{ route('actions.gallery.destroy') }}" method="POST"
            class="w-full grid grid-cols-2 grid-rows-1 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-4">
            @csrf
            @forelse ($data as $row)
                <div data-service="{{ $row->service }}"
                    ...
                    <img src="{{ asset('storage/services/' . $row->name) }}"
        ...
        <form id="uploadForm" action="{{ route('actions.gallery.create') }}" method="POST" class="w-0 h-0 overflow-hidden"
            enctype="multipart/form-data">
            @csrf
            <input id="file" type="file" name="images[]" accept="image/*" multiple />
            <div id="modal"
                class="fixed inset-0 bg-gray-950 bg-opacity-40 backdrop-blur-sm flex flex-col justify-end z-50 lg:items-center lg:justify-center hidden">
                <div class="w-full rounded-t-md flex flex-col bg-white max-h-custom lg:w-[500px] lg:rounded-md">
                    <h2 class="font-black text-gray-950 text-sm p-2 border-b border-gray-300 w-full text-center">
                        {{ __('Select service') }}
                    </h2>
                    <ul id="services" class="flex-1 overflow-auto flex-col">
                        <li tabindex="0"
                            class="px-3 py-2 relative outline-none hover:text-white hover:bg-primary focus:text-white focus:bg-primary cursor-pointer">
                            <input tabindex="-1" type="radio" name="service" value="glass facade cleaning"
                                class="opacity-0 w-full h-full absolute inset-0 pointer-events-none" />
                            <span>
                                {{ __('Glass Facade Cleaning') }}
                            </span>
                        </li>
                        <li tabindex="0"
                            class="px-3 py-2 relative outline-none hover:text-white hover:bg-primary focus:text-white focus:bg-primary cursor-pointer">
                            <input tabindex="-1" type="radio" name="service" value="stone facade cleaning in kuwait"
                                class="opacity-0 w-full h-full absolute inset-0 pointer-events-none" />
                            <span>
                                {{ __('Stone Facade Cleaning in Kuwait') }}
                            </span>
                        ...
                        <li tabindex="0"
                            class="px-3 py-2 relative outline-none hover:text-white hover:bg-primary focus:text-white focus:bg-primary cursor-pointer">
                            <input tabindex="-1" type="radio" name="service" value="exterior floor cleaning"
                                class="opacity-0 w-full h-full absolute inset-0 pointer-events-none" />
                            <span>
                                {{ __('Exterior Floor Cleaning') }}
                            </span>
...
@section('scripts')
    <script>
        ...
        const _uploadForm = document.querySelector("#uploadForm"),
        ...
        UploadAction((e) => {
            if (e.target.files.length) modal.classList.remove("hidden");
        });
        ...
        [...services.children].forEach((service) => {
            ...
            service.addEventListener("click", fn);
        ...
        modal.addEventListener("click", (e) => {
            if (e.target === modal && !modal.classList.contains("hidden")) {
                _uploadForm.querySelector("#file").value = "";
                modal.classList.add("hidden");
            }
        });
        ...
        document.querySelector("#filter").addEventListener("x-change", e => {
            const items = [...document.querySelectorAll("[data-service]")];
            if (e.target.value.trim() === "all") items.forEach(i => i.classList.remove("hidden"));
            else items.forEach(i => {
                if (i.dataset.service.trim() === e.target.value.trim()) i.classList.remove("hidden");
