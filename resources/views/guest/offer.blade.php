...
@section('content')
    ...
    <section class="mb-10 mt-8 lg:mt-10 2xl:container mx-auto overflow-hidden">
        <div class="container mx-auto p-4 2xl:p-8 flex flex-col gap-6 lg:gap-10">
            <div class="grid gap-4 grid-rows-1 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-5">
                @forelse($data as $row)
                    <img data-idx="{{ $loop->index }}" onclick="ShowSlider(event)" alt="offer {{ $row->id }}"
                        src="{{ asset('storage/offers/' . $row->name) }}"
                        class="w-full aspect-video object-cover cursor-pointer bg-[#fcfcfc] rounded-md lg:rounded-lg" />
                @empty
                    <p class="md:col-span-2 lg:col-span-3 2xl:col-span-5 font-black text-gray-950 text-lg p-4 text-center">
                        {{ __('NO OFFERS FOUND') }}
                    </p>
                @endforelse
            </div>
